...
class Neopixel:
    ...
    def traverse(self, pixel1, pixel2, rgb=(255, 0, 0), delay=0.05, clear_after=True):

   # Move one colored pixel from LED 0 → last LED.

   # :param rgb: (r, g, b) color
    #:param delay: time between steps
    #:param clear_after: whether to clear previous LED

        for i in range(self.num_leds):
            if clear_after:
                self.clear()  # turn off all LEDs

            self.set_pixel(i, rgb)  # light up current LED
            self.show()
            time.sleep(delay)
        
    def traverse_rainbow(self, delay=0.05):
        for i in range(self.num_leds):
            self.clear()

            hue = int((i / self.num_leds) * 65535)
            color = self.colorHSV(hue, 255, 255)
    ...
    def traverse_segment(self, pixel1, pixel2, coco, flash_max, on_max, shift, delay):
        
        """
        Paints color groups on to a segment, then rotates them as an animation.

        :param pixel1:    start index of the segment
        :param pixel2:    end index of the segment (inclusive)
        :param coco:      list of dicts, each with 'color' (r,g,b) and 'count' (int)
                          e.g. [{'color': (255,0,0), 'count': 3}, {'color': (0,0,255), 'count': 5}]
        :param flash_max: total duration (seconds) of the scrolling animation
        :param on_max:    total "on" time (seconds) — LEDs hold still for (on_max - flash_max)
        :param shift:     how many pixels to rotate right each step
        :param delay:     time (seconds) between each rotation step
        """
        self.clear()
        span = pixel2 - pixel1 + 1

        # --- Check that all color groups fit in the segment ---
        total = 0
        for group in coco:
            total += group['count']
        ...
        # --- Paint initial color groups side by side ---
        z = pixel1
        t= True
        ...
        while t:    
            for group in coco:
                ...
                if g > pixel2 - 1:
                    g = pixel2
                    t = False
                self.set_pixel_line(z, g, group['color'])
                z += group['count']
                
        self.show()
        
        # --- Scrolling animation: rotate right by `shift` each step ---
        steps = int(flash_max / delay)
        for k in range(steps):
            if has_new_command():
                return
            # Extract just the segment, rotate right, put it back
            segment = list(self.pixels[pixel1:pixel2 + 1])
            segment = segment[-shift:] + segment[:-shift]
            for i, val in enumerate(segment):
                self.pixels[pixel1 + i] = val
            self.show()
            time.sleep(delay)

        # --- Hold still for remaining time, then clear ---
        
        if on_max > flash_max:
            hold_steps = int((on_max - flash_max) / delay)
